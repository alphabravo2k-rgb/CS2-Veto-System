...
import { AnimatedBackground, UploadIcon, ExternalLinkIcon, CheckIcon } from '../components/SharedUI';
...
export default function TournamentDashboard() {
    ...
    useEffect(() => {
        ...
        // Fetch Base Maps
        fetch(`${SOCKET_URL}/api/maps`).then(r => r.ok ? r.json() : []).then(data => {
            if (data.length > 0) {
        ...
        // Connect Socket specifically for Match Creation
        socketRef.current = io(SOCKET_URL);
        
        socketRef.current.on('match_created', ({ roomId, keys }) => {
            // 🛡️ ARCHITECTURE FIX: Generate the new Multi-Tenant Links
            const baseUrl = `${window.location.origin}/${orgId}/${tournamentId}/veto/${roomId}`;
            setCreatedLinks({
                admin: `${baseUrl}?key=${keys.admin}`,
                teamA: `${baseUrl}?key=${keys.A}`,
                teamB: `${baseUrl}?key=${keys.B}`
            });
    ...
    const createMatch = (type) => {
        ...
        socketRef.current.emit('create_match', {
            orgId, tournamentId, // Tell the backend exactly where this belongs
            teamA, teamB, teamALogo, teamBLogo, format,
            customMapNames: format === 'custom' ? customSelectedMaps : null,
            customSequence: format === 'custom' ? customSequence : null,
            useTimer, useCoinFlip, timerDuration: useTimer ? parseInt(timerDuration) : 60,
